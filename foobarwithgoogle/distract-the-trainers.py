...
def willPlayForever(i, j):
    # First get the greatest common divisor of i and j
    gcd = fractions.gcd(i, j)
    s = (i + j) // gcd
    # Check if s is power of 2, if not, then game will play forever
    return s != (s & -s)


# The following MAXIMUM CARDINALITY MATCHING code is from
...
def solution(banana_list):
    ...
    graph, l = {}, len(banana_list)
    for i in range(l):
        for j in range(i + 1, l):
            if willPlayForever(banana_list[i], banana_list[j]):
                graph.setdefault(i, []).append(j)
                graph.setdefault(j, []).append(i)
    # for key in sorted(graph):
    #     print(key, graph[key])
    return l - len(matching(graph))
